...
am__aclocal_m4_deps = $(top_srcdir)/m4/libtool.m4 \
	...
	$(top_srcdir)/config/ax_info.m4 \
	$(top_srcdir)/config/ax_lib_oracle_oci.m4 \
...
am__libzdb_la_SOURCES_DIST = src/util/Str.c src/util/Vector.c \
	...
	src/db/sqlite/SQLiteResultSet.c \
	src/db/sqlite/SQLitePreparedStatement.c \
	src/db/sqlite/SQLiteAdapter.c src/db/oracle/OracleConnection.c \
	src/db/oracle/OracleResultSet.c \
	src/db/oracle/OraclePreparedStatement.c \
	src/db/oracle/OracleAdapter.c
...
am__v_lt_0 = --silent
am__v_lt_1 = 
libzdb_la_LINK = $(LIBTOOL) $(AM_V_lt) --tag=CC $(AM_LIBTOOLFLAGS) \
...
am__DIST_COMMON = $(srcdir)/Makefile.in $(srcdir)/zdb.pc.in \
	$(top_srcdir)/config/compile $(top_srcdir)/config/config.guess \
	$(top_srcdir)/config/config.sub \
	$(top_srcdir)/config/install-sh $(top_srcdir)/config/ltmain.sh \
	$(top_srcdir)/config/missing $(top_srcdir)/src/xconfig.h.in \
	AUTHORS COPYING README config/compile config/config.guess \
...
CPPFLAGS = -I/home/dyy/gbasegci/gci/include 
CSCOPE = cscope
CTAGS = ctags
CXX = g++
CXXCPP = g++ -E
...
CXXFLAGS = -g -O2
CYGPATH_W = echo
DBCPPFLAGS =  -I/home/dyy/gbasegci/gci/include 
DBLDFLAGS =  -L/usr/lib/x86_64-linux-gnu -lpthread -lm  -latomic -ldl -lclntsh -L/home/dyy/gbasegci/gci/lib -L/home/dyy/gbasegci/gci/relyon
DEFS = -DHAVE_CONFIG_H
DEPDIR = .deps
DLLTOOL = false
DSYMUTIL = 
...
LDFLAGS = -L/usr/lib/x86_64-linux-gnu -lpthread -lm  -ldl -latomic  -lclntsh -L/home/dyy/gbasegci/gci/lib -L/home/dyy/gbasegci/gci/relyon
LEX = flex
LEXLIB = 
...
LTLIBOBJS = 
LT_SYS_LIBRARY_PATH = 
MAKEINFO = ${SHELL} '/home/dyy/libzdb-3.2.3/config/missing' makeinfo
MANIFEST_TOOL = :
...
UNIT_TEST = 
VERSION = 3.2.3
abs_builddir = /home/dyy/libzdb-3.2.3
abs_srcdir = /home/dyy/libzdb-3.2.3
abs_top_builddir = /home/dyy/libzdb-3.2.3
abs_top_srcdir = /home/dyy/libzdb-3.2.3
ac_ct_AR = ar
ac_ct_CC = gcc
ac_ct_CXX = g++
ac_ct_DUMPBIN = 
...
install_sh = ${SHELL} /home/dyy/libzdb-3.2.3/config/install-sh
...
SUBDIRS = . $(UNIT_TEST)
DIST_SUBDIRS = . test
EXTRA_DIST = README AUTHORS CHANGES COPYING bootstrap doc  src tools config
LIBRARY_NAME = zdb
RE2CFLAGS = -i
...
libzdb_la_LDFLAGS = $(DBLDFLAGS) -version-info 14:0:1
...
src/db/sqlite/SQLitePreparedStatement.lo:  \
	src/db/sqlite/$(am__dirstamp)
src/db/sqlite/SQLiteAdapter.lo: src/db/sqlite/$(am__dirstamp)
...
src/db/oracle/OraclePreparedStatement.lo:  \
	src/db/oracle/$(am__dirstamp)
src/db/oracle/OracleAdapter.lo: src/db/oracle/$(am__dirstamp)

libzdb.la: $(libzdb_la_OBJECTS) $(libzdb_la_DEPENDENCIES) $(EXTRA_libzdb_la_DEPENDENCIES) 
	$(AM_V_CCLD)$(libzdb_la_LINK) -rpath $(libdir) $(libzdb_la_OBJECTS) $(libzdb_la_LIBADD) $(LIBS)

mostlyclean-compile:
	-rm -f *.$(OBJEXT)
	-rm -f src/db/*.$(OBJEXT)
	-rm -f src/db/*.lo
	-rm -f src/db/mysql/*.$(OBJEXT)
	-rm -f src/db/mysql/*.lo
	-rm -f src/db/oracle/*.$(OBJEXT)
	-rm -f src/db/oracle/*.lo
	-rm -f src/db/postgresql/*.$(OBJEXT)
	-rm -f src/db/postgresql/*.lo
	-rm -f src/db/sqlite/*.$(OBJEXT)
	-rm -f src/db/sqlite/*.lo
	-rm -f src/exceptions/*.$(OBJEXT)
	-rm -f src/exceptions/*.lo
	-rm -f src/net/*.$(OBJEXT)
	-rm -f src/net/*.lo
